...
def main():
    ...
    scores_val      = test_network(test_loader, model, linear_classifier)
    
    logger.info(scores_val)
...
def test_network(val_loader, model, linear_classifier):
    ...
    with torch.no_grad():
        ...
        for i, (inp, target) in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                labels  = [i for i in range(args.data_classes)]
                curr_cm = confusion_matrix(temp, pred, labels = labels)
                
                if total_cm is None:
                    total_cm = curr_cm
                else:
                    total_cm = total_cm + curr_cm
    ...
    print(total_cm)
    total_recall     = np.diag(total_cm) / np.sum(total_cm, axis = 1)
    total_precision  = np.diag(total_cm) / np.sum(total_cm, axis = 0)
    print("Class level recall")
    print(total_recall)
    print("Class level precision")
    ...
    print("Total recall, precision, f1")
    print([total_recall, total_precision, total_f1])
    ...
    if args.rank == 0:
        logger.info(
            ...
            "Loss {loss.avg:.4f}\t"
            "Acc@1 {top1.avg:.3f}\t"
            "Best Acc@1 so far {acc:.1f}".format(batch_time=batch_time, loss=losses, top1=top1, acc=best_acc))
